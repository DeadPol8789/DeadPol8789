# 🎓 Certifications & Training

This file contains my complete and organized record of completed courses, workshops, and technical training.

The main profile README only displays a selected group of the most relevant items. New credentials will be added here without redesigning the profile.

## Software Development

### Introduction to Python

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Python fundamentals

### Introduction to Java

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Java fundamentals

### Introduction to SQL

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Relational databases and SQL fundamentals

### SQL Intermediate

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Intermediate SQL

## Artificial Intelligence

### Generative AI in Practice

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Practical use of generative AI

### Introduction to LLMs

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Large language model fundamentals

### Meet Claude

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Claude AI fundamentals and practical use
  
### Brainstorm with AI

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** AI-assisted ideation

### Critical Thinking in the Age of AI

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Critical thinking, AI literacy, and responsible decision-making

### AI-Powered A/B Testing

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** AI-assisted experimentation, A/B testing, and data-driven optimization

### AI Development: Program with Agents

- **Issuer:** MoureDev / BIG school
- **Year:** 2026
- **Duration:** 6 hours
- **Area:** AI-assisted development and programming agents

### AI Prompts Fundamentals

- **Issuer:** Coddy
- **Year:** 2026
- **Area:** Prompt fundamentals

### Prompt Engineering

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Prompt design, AI interaction, and effective instruction writing

### Agentic Workflows

- **Issuer:** SoloLearn
- **Year:** 2026
- **Area:** Agentic AI workflows and task orchestration

## Workshops & Events

### Vibe-Coding & Lean Startup Workshop

- **Issuer:** TechHub FP Euroformac
- **Location:** Madrid, Spain
- **Year:** 2026
- **Area:** AI-assisted development, prototyping, and Lean Startup

---

## Credential policy

Credentials are organized using the following rule:

- **Professional certification:** listed here and highlighted in the main README
- **Relevant technical course:** listed here and highlighted only when it is one of the most relevant
- **Introductory or short course:** listed here but not highlighted individually in the main README

When certificate files are uploaded, they will be stored under `certificates/` and linked from the corresponding entry.
